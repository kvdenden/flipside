...
const PredictionMarket = z
  .object({
    ...
    expiration_date: z.number().int().describe("The expiration date of the market in UNIX timestamp format."),
...
const SYSTEM_PROMPT = `
  ...
  **Expiration Date (timestamp)**: A realistic date in UNIX timestamp format (seconds since epoch) after which the market will be invalid if unresolved.
  ...
  try {
    ...
    if (!resultContent) {
      ...
      return NextResponse.json(
        { error: "The prediction could not be processed. Please try rephrasing or simplifying the content." },
        { status: 400 }
      );
    }
